...
bool js_creator_FilterNode_drawFilter(JSContext *cx, uint32_t argc, jsval *vp)
{
    ...
    js_proxy_t *proxy = jsb_get_js_proxy(obj);
    creator::FilterNode* cobj = (creator::FilterNode *)(proxy ? proxy->ptr : NULL);
    JSB_PRECONDITION2( cobj, cx, false, "js_creator_FilterNode_drawFilter : Invalid Native Object");
    if (argc == 2) {
        creator::FilterTexture* arg0 = nullptr;
        creator::FilterTexture* arg1 = nullptr;
        do {
            if (args.get(0).isNull()) { arg0 = nullptr; break; }
            if (!args.get(0).isObject()) { ok = false; break; }
            js_proxy_t *jsProxy;
        ...
        do {
            ...
            jsProxy = jsb_get_js_proxy(tmpObj);
            arg1 = (creator::FilterTexture*)(jsProxy ? jsProxy->ptr : NULL);
            JSB_PRECONDITION2( arg1, cx, false, "Invalid Native Object");
        } while (0);
        JSB_PRECONDITION2(ok, cx, false, "js_creator_FilterNode_drawFilter : Error processing arguments");
        cobj->drawFilter(arg0, arg1);
        args.rval().setUndefined();
        return true;
    }
...
bool js_creator_FilterNode_constructor(JSContext *cx, uint32_t argc, jsval *vp)
{
    JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
    bool ok = true;
    creator::FilterNode* cobj = new (std::nothrow) creator::FilterNode();
    TypeTest<creator::FilterNode> t;
    js_type_class_t *typeClass = nullptr;
    std::string typeName = t.s_name();
    auto typeMapIter = _js_global_type_map.find(typeName);
    CCASSERT(typeMapIter != _js_global_type_map.end(), "Can't find the class type!");
    ...
    if (JS_HasProperty(cx, obj, "_ctor", &ok) && ok)
        ScriptingCore::getInstance()->executeFunctionWithOwner(OBJECT_TO_JSVAL(obj), "_ctor", args);
    return true;
}static bool js_creator_FilterNode_ctor(JSContext *cx, uint32_t argc, jsval *vp)
{
    JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
    JS::RootedObject obj(cx, args.thisv().toObjectOrNull());
    creator::FilterNode *nobj = new (std::nothrow) creator::FilterNode();
    js_proxy_t* p = jsb_new_proxy(nobj, obj);
    bool isFound = false;
    if (JS_HasProperty(cx, obj, "_ctor", &isFound) && isFound)
        ScriptingCore::getInstance()->executeFunctionWithOwner(OBJECT_TO_JSVAL(obj), "_ctor", args);
...
void js_creator_FilterNode_finalize(JSFreeOp *fop, JSObject *obj) {
    CCLOGINFO("jsbindings: finalizing JS object %p (FilterNode)", obj);
    JSContext *cx = ScriptingCore::getInstance()->getGlobalContext();
    JS::RootedObject jsobj(cx, obj);
    auto proxy = jsb_get_js_proxy(jsobj);
    if (proxy) {
        creator::FilterNode *nobj = static_cast<creator::FilterNode *>(proxy->ptr);
...
void js_register_creator_FilterNode(JSContext *cx, JS::HandleObject global) {
    jsb_creator_FilterNode_class = (JSClass *)calloc(1, sizeof(JSClass));
    jsb_creator_FilterNode_class->name = "FilterNode";
    jsb_creator_FilterNode_class->addProperty = JS_PropertyStub;
    jsb_creator_FilterNode_class->delProperty = JS_DeletePropertyStub;
    jsb_creator_FilterNode_class->getProperty = JS_PropertyStub;
    jsb_creator_FilterNode_class->setProperty = JS_StrictPropertyStub;
    jsb_creator_FilterNode_class->enumerate = JS_EnumerateStub;
    jsb_creator_FilterNode_class->resolve = JS_ResolveStub;
    jsb_creator_FilterNode_class->convert = JS_ConvertStub;
    jsb_creator_FilterNode_class->finalize = js_creator_FilterNode_finalize;
    jsb_creator_FilterNode_class->flags = JSCLASS_HAS_RESERVED_SLOTS(2);
    ...
    JS::RootedObject parent_proto(cx, jsb_cocos2d_Node_prototype);
    jsb_creator_FilterNode_prototype = JS_InitClass(
        cx, global,
        parent_proto,
        jsb_creator_FilterNode_class,
        js_creator_FilterNode_constructor, 0, // constructor
        properties,
        funcs,
        NULL, // no static properties
        st_funcs);
    ...
    // add the proto and JSClass to the type->js info hash table
    JS::RootedObject proto(cx, jsb_creator_FilterNode_prototype);
    jsb_register_class<creator::FilterNode>(cx, jsb_creator_FilterNode_class, proto, parent_proto);
    anonEvaluate(cx, global, "(function () { cc.FilterNode.extend = cc.Class.extend; })()");
}
