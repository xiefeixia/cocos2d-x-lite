...
function androidmk.generate_projectmk(prj)
  ...
  -- for cfg in project.eachconfig(prj) do
    ...
    androidmk.prjExportLibs(prj, cfg)
    androidmk.prjCFlags(prj, cfg)

    -- custom strings
    androidmk.prjCustomStrings(prj, cfg)
...
function androidmk.prjCustomStrings(prj, cfg)
  p.w('')

  if prj.amk_customstrings then
    for _, str in ipairs(prj.amk_customstrings) do
      p.w(str)
    end
  end
end
...
function androidmk.prjKind(prj, cfg)
  ...
  for _, dep in ipairs(dependencies) do
    p.w('$(call import-module, %s)', path.getrelative(prj.location, dep.project.location))
  end

  for _, mod in ipairs(prj.amk_importmodules) do
    p.w('$(call import-module, %s)', mod)
  end

  -- for _, mkpath in ipairs(prj.amk_includes) do
  --   local mkrelpath = path.getrelative(prj.location, mkpath)
  --   p.w('  include '..mkrelpath)
  -- end
end
...
function androidmk.prjDependencies(prj, cfg)
  ...
  local staticdeps = {}
  local shareddeps = {}
  local whole_staticdeps = {}

  local dependencies = premake.config.getlinks(cfg, "dependencies", "object")
  for _, dep in ipairs(dependencies) do
    if dep.kind == premake.STATICLIB then
      table.insert(staticdeps, dep.project.name)
    else
      table.insert(shareddeps, dep.project.name)
    end
  end
  ...
  for _, v in ipairs(cfg.amk_whole_staticlinks) do
    table.insert(whole_staticdeps, v)
  end

  if #whole_staticdeps > 0 then
    p.w('LOCAL_WHOLE_STATIC_LIBRARIES := %s', table.implode(whole_staticdeps, '', ' \\\n', ' '))
  end
...
function androidmk.prjExportLibs(prj, cfg)
  if prj.amk_exportlibs and #prj.amk_exportlibs > 0 then
    local libs = {}
    for _, lib in ipairs(prj.amk_exportlibs) do
      table.insert(libs, "-l"..lib)
    end
    p.w("LOCAL_EXPORT_LDLIBS := %s", table.implode(table.translate(libs, p.esc), '', '', ' '))
  end
end
...
function androidmk.prjCFlags(prj, cfg)
  ...
  if #flags > 0 then
    ...
    -- p.w('  LOCAL_EXPORT_CFLAGS := %s', table.implode(table.translate(flags, p.esc), '', '', ' '))
  end
