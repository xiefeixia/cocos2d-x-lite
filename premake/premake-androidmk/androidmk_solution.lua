...
function androidmk.generate_applicationmk(sln)
  ...
  p.w('APP_STL := gnustl_static')
  p.w('')
  p.w('# Uncomment this line to compile to armeabi-v7a, your application will run faster but support less devices')
  p.w('APP_ABI := armeabi-v7a')
  p.w('')
  p.w('APP_CPPFLAGS := -frtti -DCC_ENABLE_CHIPMUNK_INTEGRATION=1 -DCC_ENABLE_BULLET_INTEGRATION=1 -std=c++11 -fsigned-char')
  p.w('APP_LDFLAGS := -latomic')
  p.w('')
  p.w('USE_ARM_MODE := 1')
  p.w('')
  p.w('ifeq ($(NDK_DEBUG),1)')
  p.w('  APP_CPPFLAGS += -DCOCOS2D_DEBUG=1')
  p.w('  APP_OPTIM := debug')
  p.w('else')
  p.w('  APP_CPPFLAGS += -DNDEBUG')
